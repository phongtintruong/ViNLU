...
def main(args):
    ...
    # Check if contrastive learning is enabled and load the corresponding dataset
    if args.use_contrastive_learning:
        # Load triplet-based contrastive learning dataset
        train_dataset = load_and_cache_triplet_examples(args, tokenizer, mode="train")
        dev_dataset = load_and_cache_triplet_examples(args, tokenizer, mode="dev")
        test_dataset = load_and_cache_triplet_examples(args, tokenizer, mode="test")
    else:
        # Load standard joint learning dataset
...
if __name__ == "__main__":
    ...
    parser.add_argument("--logging_steps", type=int, default=200, help="Log every X update steps.")
